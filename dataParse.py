...
'''Evan added this to work for the backend'''
def insert_cofa_and_tests_from_dict(data: dict):
    ...
    insert_query = """
        INSERT INTO cofas(
            product_name, product_description, customer_material_number,
            quantity, batch_number, manufacturing_date, delivery_number,
            delivery_item, delivery_date, receiving_date, oil_additives,
            customer_number, purchase_order_number, tank_number, vehicle_number,
            shelf_life_exp_date, signature
    ...
    insert_cofa_tests_sql = """
        INSERT INTO cofa_tests (
            cofa_id, test_name, appearance, value, uom, min_spec, max_spec
    ...
    length = min(len(test_names), len(appearances), len(values_), len(uoms), len(min_specs), len(max_specs))

    for i in range(length):
        cursor.execute(insert_cofa_tests_sql, (
            new_cofa_id,
...
if __name__ == "__main__":
    ...
    insert_cofa_and_tests("Hitec1_prompt1.json")
