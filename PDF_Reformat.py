...
pdf_path = "Raw Material (1).pdf"  # Update with your PDF file path
...
for image_path in image_list:
    ...
    for x1, x2 in column_indices:
        for y1, y2 in row_indices:
            ...
            # Draw red horizontal line through the middle
            draw.line([(x1, y_mid), (x2, y_mid)], fill="red", width=2)
    ...
    output_filename = f"output_with_lines_{image_path}"
    output_image.save(output_filename)

    plt.figure(figsize=(12, 10))
    plt.imshow(output_image)
    plt.title(f"Detected Columns and Rows with Red Lines - {image_path}")
    plt.axis("off")
    plt.show()

    print(f"Saved output image with red lines: {output_filename}")
